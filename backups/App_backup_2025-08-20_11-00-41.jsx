...
import { fetchRecentAudioFiles, getAudioMetadata, getThumbnailUrl, fetchPlaylists } from './services/qdnService';
  ...
  useEffect(() => {
    const loadContent = async () => {
      try {
        ...
        const playlistData = await fetchPlaylists(20, 0);
          ...
          <main className="main-content">
            <div className="content-area">
              <Routes>
                        ...
                        {isLoading ? (
                          ...
                          playlists.map((playlist) => (
                            <AudioCard
                              ...
                              audio={{
                                id: `${playlist.name}_${playlist.service}_${playlist.identifier}`,
                                title: playlist.title || extractTitleFromIdentifier(playlist.identifier) || 'Untitled Playlist',
                                artist: playlist.name,
                                uploader: playlist.name,
                                thumbnail: getThumbnailUrl(playlist.name, playlist.identifier),
                                ...playlist
                              }}
                              isPlaylist={true}
                              onClick={() => {
                                // Navigeerimine tehakse Link komponendiga
                                window.location.hash = `#/playlist/${playlist.identifier}`;
                              }}
                            />
                          ))
                        )}
                ...
                <Route path="/upload" element={isLoggedIn ? <AddMusicForm currentUser={currentUser} onMusicAdded={handleMusicAdded} /> : <Navigate to="/" />} />
              </Routes>
            </div>
          </main>
